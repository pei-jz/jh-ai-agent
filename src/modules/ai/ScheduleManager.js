...
/**
 * Which day of `now`'s month a monthly schedule falls on.
 *
 * CLAMPED to the month's length, so "the 31st" runs on the 28th of February
 * instead of silently skipping the month — a monthly schedule that quietly does
 * not run is worse than one that runs a few days early.
 *
 * @param {number|'last'} dayOfMonth
 * @param {Date} now
 */
...
class ScheduleManager {
     ...
     *   "monthly"  — fire at s.time on s.dayOfMonth (1-31, or 'last') each month
     */
    _shouldFire(s, now) {
        ...
        if (type === 'monthly') {
            ...
            return now.getDate() === monthDay(s.dayOfMonth ?? 1, now) && s.time === hhmm;
        }
