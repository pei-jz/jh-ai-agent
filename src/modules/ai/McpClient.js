...
export class McpClient {
    ...
    async _doStart() {
        ...
        try {
            ...
            // Documents the server exposes for reading — stdio servers commonly
            // publish files, though a plain server may publish none.
            // Servers that declare no `resources` capability are skipped
            // without a round-trip.
            try {
                this.resources = this.capabilities?.resources
                    ? ((await this.request('resources/list', {}, 5000))?.resources || [])
                    : [];
            } catch (_) {
                this.resources = [];
            }
