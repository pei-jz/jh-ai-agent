...
export const TOOL_DEFINITIONS = [
    ...
    },
    {
        name: 'read_office',
        isSafe: true,
        description: 'Read an Office document (.xlsx/.xlsm/.xls/.ods/.docx/.pptx) as Markdown. USE THIS INSTEAD OF read_file for Office files — read_file returns binary garbage for them. Workbooks start with an index of every sheet (name, rows, cols); if the whole workbook does not fit, the response names the sheets it left out — call again with `sheet` to read one of those in full. Word comes back as paragraphs, PowerPoint as per-slide bullets (speaker notes are NOT included).',
        parameters: {
            type: 'object',
            properties: {
            ...
            required: ['path', 'sheet', 'include_images', 'max_chars'],
            additionalProperties: false
        }
    },
    {
        ...
        parameters: {
            type: 'object',
            properties: {
                app: { type: ['string', 'null'], description: 'Optional. Limit to one app name.' }
            },
            required: ['app'],
            additionalProperties: false
        }
    },
    {
        name: 'read_resource',
        isSafe: true,
        description: 'Read one live document published by a connected app. Pass the full "app::uri" reference shown by list_resources; a bare uri works only when a single app publishes it.',
        parameters: {
            type: 'object',
            properties: {
                uri: { type: 'string', description: 'Resource reference, e.g. "jheditor::doc://current".' }
            },
            required: ['uri'],
            additionalProperties: false
        }
    },
    {
        name: 'write_docx',
        isSafe: false,
        description: 'Create a Word (.docx) document from light Markdown. Supports "# Heading" (levels 1-6), "- item" / "1. item" lists, blank-line-separated paragraphs, and **bold** / *italic*. Use this when the user asks for a Word document — do NOT hand them Markdown and expect them to convert it.',
        parameters: {
            type: 'object',
            properties: {
                path: { type: 'string', description: 'Destination .docx path.' },
                markdown: { type: 'string', description: 'The document body as light Markdown.' }
            },
    ...
    },
    {
        ...
        parameters: {
            type: 'object',
            properties: {
                path: { type: 'string', description: 'Existing .xlsx to edit.' },
                edits: {
                    type: 'array',
    ...
    },
    {
        ...
        parameters: {
            ...
            required: ['query', 'kind', 'path', 'include_glob', 'max_results'],
            additionalProperties: false
        }
    },
    {
