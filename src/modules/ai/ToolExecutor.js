...
// The shell run_command actually spawns, fetched once per process. Cached at the
// MODULE level so every ToolExecutor (including the throwaway used by the static
// helper) advertises the same thing — a description that changed mid-session
// would also invalidate the provider's prompt cache.
let SHELL_INFO = null;

async function ensureShellInfo() {
    if (SHELL_INFO) return SHELL_INFO;
    try {
        SHELL_INFO = await invoke('get_shell_info');
    } catch (e) {
        // An older backend has no such command. Saying nothing beats guessing.
        console.warn('Shell info unavailable — run_command description stays generic:', e);
    }
    return SHELL_INFO;
}
...
export class ToolExecutor {
    constructor() {
        ...
        // Images a tool produced this step (Office diagrams, screenshots …).
        // Tool results are text-only on every provider, so a tool cannot hand an
        // image to the model directly — it parks it here and the agent loop
        // attaches it to the next request. Drained once, never re-sent.
        this.pendingImages = [];
        ...
        // Whether finish_task / present_result / ask_user come along with an
        // allowlist. Chat turns this off — see setToolAllowlist.
        this._allowAgentControl = true;
     ...
     * @param {boolean} [opts.agentControl] false → do NOT add the control tools.
     *   Chat has no task to finish, no Result Contract to deliver and nowhere to
     *   pause: offering finish_task there made the model spend a turn "finishing"
     *   instead of answering, and the user saw a tool trace and no reply.
     */
    setToolAllowlist(allowedNames, { includeTaskTools = false, agentControl = true } = {}) {
        this._allowAgentControl = agentControl;
    ...
    getActiveToolDefinitions() {
        ...
        // Name the real shell so the model doesn't write bash for PowerShell.
        return decorateRunCommand(defs, SHELL_INFO);
    }

    /**
     * Take the images tools produced since the last call, emptying the queue.
     * @returns {Array<{data:string, source:string}>} data URLs plus their origin
     */
    drainImages() {
    ...
    async startSession(workspacePath) {
        // Before the first LLM call, so run_command's description is already
        // shell-accurate on step 1 (and stays byte-identical afterwards).
        await ensureShellInfo();
