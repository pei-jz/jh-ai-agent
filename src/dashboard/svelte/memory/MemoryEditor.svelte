<!--
  MemoryEditor — view, edit and delete the agent's long-term memory.

  Was svelte/config/MemoryTab.svelte. It moved out of Settings with the rest of
  the memory surface (docs/design/information-architecture.md §7 step 4):
  reviewing what the agent believes is not a setting, and the reading half of the
  same subject was in a THIRD place (the Dashboard's right pane). Both halves now
  live under svelte/memory/.

  The component itself is unchanged — only its imports moved up one level.
...
<script>
    ...
    /**
     * A stored date, with its weekday.
     *
     * Facts carry a plain `YYYY-MM-DD`, and a bare date does not answer the
     * question anyone actually asks of this column — "was that a working day, was
     * that the day I changed X". The weekday is one glyph and it answers it.
     * The locale comes from the app's, so this reads 月/火 in Japanese and
     * Mon/Tue in English without a second format.
     */
    const withWeekday = (iso) => {
        const raw = String(iso || '').trim();
        if (!raw) return '';
        const d = new Date(raw.length <= 10 ? `${raw}T00:00:00` : raw);
        if (Number.isNaN(d.getTime())) return raw;         // unparsable: show as stored
...
<div class="card settings-card cfg-tab-card">
    ...
    <div class="provider-card-fields">
        <!--
          No workspace field here.

          MemoryRoot's header carries ONE selector that drives both halves of the
          destination (docs/design/information-architecture.md §7 step 4). A
          second field inside the editor is the same duplication the Dashboard
          had — two inputs for one concept, free to disagree — and it was left
          behind by the move out of Settings, where this component was the only
          thing on screen and had to ask.
        -->
                            ...
                            <tbody>
                                {#each factList as f, i (i)}
                                    <tr>
                                        ...
                                        <td class="cfg-mem-meta">{withWeekday(f.date)}</td>
                        ...
                        <table class="rv-table cfg-mem-table">
                            <tbody>
                                {#each episodeList as e, i (i)}
                                    <tr>
                                        <td class="cfg-mem-meta">{withWeekday(e.date)}</td>
                                        <td class="cfg-mem-fact">
