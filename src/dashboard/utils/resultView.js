...
/**
 * @param {HTMLElement} rootEl
 * @param {string} [baseDir] workspace root. Tool telemetry reports paths the way
 *   the agent wrote them — often workspace-relative — and the OS opener needs an
 *   absolute one, which is why clicking a step's file said "file not found".
 */
/** Absolute already? Otherwise join onto the workspace root. */
export function resolveAgainst(path, baseDir = '') {
    const p = String(path || '').trim();
    if (!p) return p;
    // "C:\…", "C:/…", "/…", "\\server\share"
    const absolute = /^[a-zA-Z]:[\\/]/.test(p) || p.startsWith('/') || p.startsWith('\\\\');
    if (absolute || !baseDir) return p;
    const base = String(baseDir).replace(/[\\/]+$/, '');
    return `${base}/${p.replace(/^\.\//, '').replace(/^[\\/]+/, '')}`;
}

/**
 * Open ONE path in the OS default application.
 *
 * The counterpart to attachFileOpenHandlers for callers that already know which
 * path was activated — a Svelte component with an `onclick`, rather than a
 * container that has to be scanned for `[data-open-path]` after every render.
 * Migrated components use this; the vanilla views keep the scanner.
 *
 * @param {string} path may be workspace-relative
 * @param {string} [baseDir] workspace root
 * @returns {Promise<boolean>} whether the open succeeded
 */
export async function openPathInDefaultApp(path, baseDir = '') {
    const resolved = resolveAgainst(path, baseDir);
    if (!resolved) return false;
    try {
        await invoke('open_path_default', { path: resolved });
        return true;
    } catch (err) {
        console.error('Failed to open path:', resolved, err);
        return false;
    }
}

export function attachFileOpenHandlers(rootEl, baseDir = '') {
    if (!rootEl) return;
    rootEl.querySelectorAll('[data-open-path]').forEach(el => {
        ...
        el.addEventListener('click', async (e) => {
            ...
            if (!raw) return;
            const path = resolveAgainst(raw, baseDir);
            try {
...
export function ensureResultViewStyles() {
    ...
    style.id = 'rv-styles';
    style.textContent = `
        .rv-summary, .rv-files-title { color: var(--ink); }
        ...
        .rv-summary .rv-code { background: var(--surface-sunken); padding: 10px; border-radius: var(--r-2);
            overflow-x: auto; font-family: var(--font-mono); font-size: 12px; margin: 8px 0; }
        .rv-summary code { background: var(--surface-sunken); padding: 1px 5px; border-radius: var(--r-2);
            font-family: var(--font-mono); font-size: 12px; }
        .rv-summary .rv-quote { border-left: 3px solid var(--accent); padding-left: 10px;
            color: var(--ink-soft); margin: 8px 0; }
        .rv-summary .rv-hr { border: none; border-top: 1px solid var(--line); margin: 12px 0; }
        .rv-table { border-collapse: collapse; width: 100%; margin: 8px 0; font-size: 12px; }
        .rv-table th, .rv-table td { border: 1px solid var(--line); padding: 6px 10px; text-align: left;
            vertical-align: top; }
        .rv-table th { background: var(--surface-sunken); color: var(--ink-soft); font-weight: 600; }
        ...
        .rv-empty { color: var(--ink-faint); font-size: 12px; padding: 8px 0; }
        /* Sectioned result view */
        .rv-section-label { font-weight: 700; font-size: 12px; color: var(--ink-soft);
        ...
        .rv-chip { display: inline-flex; align-items: center; gap: 4px; font-size: 11px;
            ...
            border: 1px solid var(--line); border-radius: var(--r-pill); padding: 2px 9px; white-space: nowrap; }
        .rv-details { margin: 6px 0 14px; border: 1px solid var(--line); border-radius: var(--r-2);
        ...
        .rv-details > summary { cursor: pointer; padding: 7px 10px; font-size: 12px; font-weight: 600;
            color: var(--ink-soft); user-select: none; list-style: revert; }
        .rv-details[open] > summary { border-bottom: 1px solid var(--line); }
        .rv-details > .rv-summary, .rv-details > .rv-detail-h { padding: 0 10px; }
        .rv-detail-h { font-weight: 600; font-size: 12px; color: var(--ink-soft); margin: 10px 0 4px; }
    `;
