...
// stream is no longer needed (per the "Complete後は実行中情報は不要" note).
...
export class MonitorView {
    ...
    _snapshotLiveState() {
        ...
        const hasContent = (pending && pending.innerHTML.trim()) || (live && live.innerHTML.trim());
        if (!hasContent) { _liveSnapshots.delete(id); return; }
        _liveSnapshots.set(id, {
            pendingHTML: pending ? pending.innerHTML : '',
            pendingDisplay: pending ? (pending.style.display || 'flex') : 'none',
            liveHTML: live ? live.innerHTML : '',
            liveDisplay: live ? (live.style.display || 'block') : 'none',
        });
    }
    ...
    _restoreLiveState(taskId) {
        ...
        const pending = document.getElementById('result-pending');
        const live = document.getElementById('result-live');
        if (pending && snap.pendingHTML) { pending.innerHTML = snap.pendingHTML; pending.style.display = snap.pendingDisplay; }
        if (live && snap.liveHTML) { live.innerHTML = snap.liveHTML; live.style.display = snap.liveDisplay; }
        return true;
    }
    ...
    _flushReplay() {
        ...
        // Still running and no snapshot restored the request bubble → show the
        // task's request so the header isn't blank while it works. Only for the
        // first run (no completed runs yet); later runs' requests belong to the
        // steer/continue that started them and are already handled elsewhere.
        if (status === 'running') {
            const pending = document.getElementById('result-pending');
            if (pending && !pending.innerHTML.trim() && this.resultSummaries.length === 0) {
                const t = (this.tasks || []).find(t => t.id === this.selectedTaskId);
                if (t?.prompt) this._showPendingUser(t.prompt);
            }
        }
