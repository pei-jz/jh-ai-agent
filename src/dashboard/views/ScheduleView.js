...
function nextRunText(schedule) {
    ...
    if (type === 'monthly') {
        ...
        for (let i = 0; i < 4; i++) {
            ...
            const d = new Date(probe.getFullYear(), probe.getMonth(), monthDay(schedule.dayOfMonth ?? 1, probe), h, m, 0, 0);
...
const DOM_OPTIONS = [...Array.from({ length: 31 }, (_, i) => String(i + 1)), 'last'];
...
export class ScheduleView {
    constructor() {
        ...
        // "+ New" used to write the schedule straight to storage and hand it to
        // the manager — an unnamed, promptless entry appeared as a registered
        // recurring task before the user had typed anything. A new schedule is a
        // DRAFT: it lives here only, and Save is what registers it.
        this._draft = null;
    }

    /** The list as shown: the unsaved draft, if any, then the stored ones. */
    _visibleSchedules() {
        return this._draft ? [this._draft, ...this.schedules] : this.schedules;
    }

    /** The schedule being edited, draft included. */
    _editing() {
        if (this._draft && this._draft.id === this._editingId) return this._draft;
        return this.schedules.find(x => x.id === this._editingId) || null;
    }

    render() {
        return `
            <style>
                .sch-layout { display: flex; gap: 16px; height: calc(100vh - var(--titlebar-height) - 60px); }
                .sch-list-panel {
                    width: 280px;
                    min-width: 220px;
            ...
            return `
                <div class="sch-item ${isSelected ? 'selected' : ''}" data-sch-id="${s.id}">
                    <div class="sch-item-top">
                        ...
                        <span style="font-size:10px;color:${isDraft ? 'var(--warning)' : 'var(--text-tertiary)'};margin-left:auto">${isDraft ? 'Unsaved' : (s.enabled ? 'On' : 'Off')}</span>
                    </div>
                    <div class="sch-days-row">${daysHtml}</div>
                    <div class="sch-prompt-preview">${escapeHtml(s.name || s.prompt || '(untitled)')}</div>
                    <div class="sch-next">${isDraft ? 'Not registered until you save' : `Next: ${nextRunText(s)}`}</div>
                </div>
            `;
        }).join('');
    }
    ...
    _renderDetail() {
        const s = this._editingId ? this._editing() : null;
            ...
            <div class="sch-detail-body">
                ...
                <div class="sch-field">
                    ...
                    <div class="sch-type-group">
                        ...
                        <button class="sch-type-btn ${type === 'monthly'  ? 'selected' : ''}" data-type="monthly">Monthly</button>
                        <button class="sch-type-btn ${type === 'once'     ? 'selected' : ''}" data-type="once">Once</button>
                    </div>
                </div>

                <!-- Monthly section -->
                <div class="sch-field" id="sch-section-monthly" ${type !== 'monthly' ? 'style="display:none"' : ''}>
                    <label>Day of the month</label>
                    <div class="sch-time-row">
                        <select class="sch-select" id="sch-day-of-month" style="width:auto">
                            ${DOM_OPTIONS.map(v =>
                                `<option value="${v}" ${String(s.dayOfMonth ?? 1) === v ? 'selected' : ''}>${domLabel(v)}</option>`
                            ).join('')}
                        </select>
                        <input type="time" class="sch-time-input" id="sch-time-monthly" value="${escapeHtml(s.time || '09:00')}">
                    </div>
                ...
                <!-- Days picker. Meaningless for "once" and for "monthly", whose
                     day comes from the calendar rather than the week. -->
                <div class="sch-field" id="sch-section-days" ${(type === 'once' || type === 'monthly') ? 'style="display:none"' : ''}>
    ...
    _bindDetail() {
        ...
        document.querySelectorAll('.sch-type-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                document.getElementById('sch-section-monthly').style.display  = t === 'monthly'  ? '' : 'none';
                document.getElementById('sch-section-days').style.display     = (t === 'once' || t === 'monthly') ? 'none' : '';
            });
        });
        ...
        document.getElementById('btn-save-schedule')?.addEventListener('click', () => {
            ...
            s.scheduleType = type;
            // Monthly has its own time input, so the hidden "fixed" one must not
            // be the value that gets saved.
            s.time = (type === 'monthly'
                ? document.getElementById('sch-time-monthly')?.value
                : document.getElementById('sch-time')?.value) || '09:00';
            s.dayOfMonth = document.getElementById('sch-day-of-month')?.value || 1;
            s.days = selectedDays;
            ...
            // Saving is what REGISTERS a draft. A schedule with no instruction has
            // nothing to run, so it stays a draft rather than becoming a silently
            // dead entry in the list.
            if (this._draft === s) {
                if (!s.prompt) { alert('Enter a prompt before saving — a schedule with no instruction has nothing to run.'); return; }
        ...
        document.getElementById('btn-run-now')?.addEventListener('click', async () => {
            ...
            if (this._draft === s) { alert('Save the schedule first, then run it.'); return; }
        ...
        document.getElementById('btn-delete-schedule')?.addEventListener('click', () => {
            ...
            // ever registered.
            if (this._draft && this._draft.id === this._editingId) {
                this._draft = null;
                this._editingId = null;
                this._refreshList();
                this._refreshDetail();
    ...
    init() {
        document.getElementById('btn-new-schedule')?.addEventListener('click', () => {
            ...
            // NOT persisted and NOT handed to the manager: it is a draft until
            // Save. Registering on "+ New" put an empty schedule in the list as a
            // live recurring task.
            this._draft = newSch;
