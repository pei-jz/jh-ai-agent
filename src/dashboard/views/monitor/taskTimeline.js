...
/**
 * Merge late-arriving per-file descriptions onto a run's file rows, in place.
 *
 * `rows` is the authority on WHICH files a run touched; the patch only supplies
 * descriptions for them, so a patch entry with no matching row is ignored. Both
 * the live view (MonitorView) and the replay reducer go through this, so a
 * reloaded run and a live one cannot disagree.
 *
 * @returns {boolean} true when at least one row changed
 */
export function applyFileDescriptions(rows, patch) {
    if (!Array.isArray(rows) || !Array.isArray(patch) || patch.length === 0) return false;
    const byPath = new Map(patch.map(f => [String(f?.path || ''), f]));
    let changed = false;
    for (const row of rows) {
        const p = byPath.get(String(row?.path || ''));
        if (!p || !p.description || p.description === row.description) continue;
...
export class TaskTimeline {
    ...
    /**
     * Close the open question WITHOUT claiming it was answered.
     *
     * Used by replay when the run that asked has ended: the question is history and
     * must stop rendering as a live prompt, but saying "answered" would be a lie
     * when the user simply walked away. The renderer distinguishes the two.
     */
    closeAsk() {
        const open = this._items.find(i => i.kind === 'ask' && !i.answered);
        if (!open) return null;
    ...
    /**
     * Patch the file table of the most recent run in place.
     *
     * Per-file descriptions are generated by an LLM call that deliberately does
     * NOT gate completion, so they can arrive after `pushRun`. Patching keeps the
     * exchange as ONE run item — emitting a second completion for them would list
     * the same work twice.
     *
     * @returns {boolean} true when a run was patched (caller should re-render)
     */
    patchRun(files) {
        const idx = findLastIndex(this._items, i => i.kind === 'run');
        if (idx < 0) return false;
        const run = this._items[idx];
        const changed = applyFileDescriptions(run.files, files);
        if (changed) this._touch(run);
        return changed;
    }
 ...
 */
/** Index of the last element matching `pred`, or -1. */
...
export function buildTimeline(logs, opts = {}) {
    ...
    for (const l of (Array.isArray(logs) ? logs : [])) {
        ...
        } else if (l.event === 'result_update' && Array.isArray(d.files)) {
            // Late-arriving file descriptions for the run just pushed. Replaying
            // these is what keeps the table populated after a reload — the
            // `complete` entry in the log store predates them.
            tl.patchRun(d.files);
        ...
        // itself becomes the next run's request, which is why an answered card is
        // then pruned rather than kept alongside it.)
        // …but NOT on the teardown bookkeeping the loop emits after it has already
        // broken on the question (memory writes, learned cards). Those come after
        // the ask and would close a card the run is still genuinely waiting on.
        if (l.event === 'status' && d.status === 'running' && d.phase !== 'teardown') tl.resolveAsk('');
    }

    // A question left open by the loop is only genuinely live if the run is still
    // paused on it. ask_user BREAKS the agent loop, so a `complete` always follows —
    // meaning every task that ever asked anything used to reopen with a highlighted,
    // clickable prompt for a run that had ended long ago. That is the bug this closes.
    //
    // Deliberately NOT closed when the question is the last thing in the log: there
    // the run really is waiting, and the prompt must stay actionable.
    // `complete` is NOT evidence that a question is dead. ask_user breaks the
    // loop, so the run that asked always completes — its result summary, its
    // memory writes and its `complete` all arrive while the task sits parked on
    // the question. Closing on `complete` therefore closed EVERY question as
    // "未回答のまま終了しました", which also replaced its answer buttons with a
    // bare "回答する" fallback: the affordance the user had was the degraded one.
    //
    // What actually shows the task moved on is work happening after that: a new
    // turn (thought / tool call / a running status that is not this run's own
    // teardown), a terminal error, or a SECOND completion. Absent those, the
    // question is the last thing that happened and must stay answerable — which
    // is what the rule below always meant to say.
    const lastAsk = findLastIndex(logs, l => l?.event === 'ask'
        || (l?.event === 'status' && l?.data?.status === 'waiting'));
    if (lastAsk >= 0) {
        const after = logs.slice(lastAsk + 1);
        const movedOn = after.some(l => l?.event === 'thought' || l?.event === 'tool_call'
            || (l?.event === 'status' && l?.data?.status === 'running' && l?.data?.phase !== 'teardown'))
            || after.some(l => l?.event === 'error' && l?.data?.terminal)
            || after.filter(l => l?.event === 'complete').length > 1;
        if (movedOn) tl.closeAsk();
    }
